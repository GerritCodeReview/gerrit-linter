...
func main() {
	...
	if *authFile == "" && *gcpServiceAccount == "" {
		log.Fatal("must set --auth_file or --gcp_service_account")
	}
	...
	if *authFile != "" {
		...
		auth, err := NewGCPServiceAccount(*gcpServiceAccount)
		if err != nil {
			log.Fatal(err)
		}
	...
	// aren't redirected properly. Also, this avoid spamming logs with
	// failure messages.
